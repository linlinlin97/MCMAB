...
class NeuralTS_agent():
    ...
    '''
    def __init__(self,M,K,N,T,m,nu=1,estimator=None,criterion=criterion,reg=1,feature_transform=False,sigma=None,order="concurrent",exp_episode=2, approximate = False, B_max = None, log_log = False, normalize = True, lr = 10**(-4)):
    ...
    def receive_reward(self, i, t, A, obs_R, X):
        ...
        if self.order == "concurrent" and self.approximate == True and i == self.M-1:
          ...
          for k in range(0, self.M*self.K-1):
            ...
            g = [sum_grad + grad for sum_grad, grad in zip(g, gradients)]
          gradients = torch.autograd.grad(outputs=f_t_batch[self.M*self.K-1], inputs=self.estimator.parameters())
          g = [sum_grad + grad for sum_grad, grad in zip(g, gradients)]
          g=flatten(g)
          g=g/(np.sqrt(self.m))
          # g=g/(np.sqrt(self.m))/(self.M*self.K)
        ...
        if self.order == "sequential" and self.approximate == True and t == self.T-1:
          ...
          g = [torch.zeros_like(param) for param in self.estimator.parameters()]
          for k in range(0, self.T*self.K-1):
            # Compute gradients for f_t[k][0] with respect to the model parameters
            gradients = torch.autograd.grad(outputs=f_t_batch[k], inputs=self.estimator.parameters(), retain_graph=True)
            # Accumulate the gradients
            g = [sum_grad + grad for sum_grad, grad in zip(g, gradients)]
          gradients = torch.autograd.grad(outputs=f_t_batch[self.T*self.K-1], inputs=self.estimator.parameters())
          g = [sum_grad + grad for sum_grad, grad in zip(g, gradients)]
          g=flatten(g)
          g=g/(np.sqrt(self.m))
          # g=g/(np.sqrt(self.m))/(self.M*self.K)
        ...
        if self.approximate == False:
          ...
          for k in range(0, self.K-1):
            ...
            # Accumulate the gradients
            g = [sum_grad + grad for sum_grad, grad in zip(g, gradients)]
          gradients = torch.autograd.grad(outputs=f_t_batch[self.K-1], inputs=self.estimator.parameters())
          g = [sum_grad + grad for sum_grad, grad in zip(g, gradients)]
          g=flatten(g)
          g=g/(np.sqrt(self.m))
          # g=g/(np.sqrt(self.m))/(self.M*self.K)
